...
(function () {
  ...
  function initSplashCursor(container, options) {
    ...
    for (k in (options || {})) config[k] = options[k];
    ...
    if (!ctx) {            // no WebGL — leave the section as plain background
      canvas.remove();
      return;
    }
    var gl = ctx.gl;
    ...
    function getWebGLContext(canvasEl) {
      ...
      var glCtx = null;
      try {
        glCtx = canvasEl.getContext('webgl2', params);
      ...
      return {
        gl: glCtx,
        ext: { formatRGBA: formatRGBA, formatRG: formatRG, formatR: formatR,
    ...
    function addKeywords(source, keywords) {
      ...
      var str = '';
      keywords.forEach(function (kw) { str += '#define ' + kw + '\n'; });
      return str + source;
    }
    ...
    function createProgram(vertexShader, fragmentShader) {
      ...
      if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
        console.warn('[splash-cursor]', gl.getProgramInfoLog(program));
      }
      return program;
    }
    ...
    function updateColors(dt) {
      if (!config.RAINBOW_MODE) return;   // fixed brand colour, nothing to cycle
    ...
    function start() {
      if (running) return;
      running = true;
      lastUpdateTime = Date.now();   // avoid one huge dt after being paused
      rafId = requestAnimationFrame(updateFrame);
    }
    ...
    container.addEventListener('mousemove', function (e) {
      ...
      if (!firstMove) {
        updatePointerMoveData(pointer, p.x, p.y, generateColor());
        firstMove = true;
      } else {
    ...
    container.addEventListener('touchstart', function (e) {
      ...
      for (var i = 0; i < touches.length; i++) {
        var p = localPos(touches[i].clientX, touches[i].clientY);
    ...
    document.addEventListener('visibilitychange', function () {
      ...
      else if ('IntersectionObserver' in window === false) start();
    });
  }
  ...
  function boot() {
    ...
    var nodes = document.querySelectorAll('[data-splash-cursor]');
    for (var i = 0; i < nodes.length; i++) {
      try {
        initSplashCursor(nodes[i], { COLOR: nodes[i].getAttribute('data-splash-color') || '#F5C518' });
      } catch (e) {
